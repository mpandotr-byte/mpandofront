...
const SaleEditModal = ({
  ...
  useEffect(() => {
    const fetchProjectDetails = async () => {
      ...
      try {
        const data = await api.get(`/projects/${saleData.proje_id}`);
    ...
    if (isOpen && saleData?.proje_id) {
      fetchProjectDetails();
    }
  }, [saleData?.proje_id, isOpen]);
  ...
  // Modal ilk açıldığında veya veriler geldiğinde mevcut unit_id'ye göre blok/daire seç
  useEffect(() => {
    if (isOpen && saleData?.unit_id && blocks.length > 0) {
      // Önce hangi blokta olduğunu bulalım
      let foundBlockId = '';
      let foundUnitId = String(saleData.unit_id);

      for (const block of blocks) {
        let unitInThisBlock = false;
        if (block.floors) {
          for (const floor of block.floors) {
            if (floor.units && floor.units.some(u => String(u.id) === foundUnitId)) {
              unitInThisBlock = true;
              break;
            }
          }
        }
        if (unitInThisBlock) {
          foundBlockId = String(block.id);
          break;
        }
      }
    ...
    } else if (isOpen && !saleData?.unit_id) {
      // Eğer unit_id yoksa seçimleri sıfırla
      setSelectedBlockId('');
      setSelectedUnitId('');
    }
  }, [isOpen, saleData?.unit_id, blocks]);
                        ...
                        {contractFile ? (
                          ...
                          <>
                            ...
                            <p className="text-xs font-semibold text-slate-600">
                              {saleData.contract_file ? 'Dosyayı Güncelle' : 'Dosya Yüklemek İçin Tıklayın'}
                            </p>
                            {saleData.contract_files && <p className="text-[10px] text-blue-500 mt-1">Mevcut bir dosya var</p>}
                          </>
                        )}
                        <input
                          type="file"
