...
export default function SubDashboard() {
    ...
    const stats = [
        { label: 'Aktif İş Sayısı', value: '4', icon: <Hammer />, color: 'text-indigo-600', bg: 'bg-indigo-50' },
        ...
        { label: 'Bekleyen Hakediş', value: '450K ₺', icon: <Clock />, color: 'text-amber-600', bg: 'bg-amber-50' },
        { label: 'Bekleyen Ödeme', value: '280K ₺', icon: <Wallet />, color: 'text-rose-600', bg: 'bg-rose-50' },
                ...
                <div className="px-4 sm:px-6 md:px-8 pb-12 pt-4 md:pt-6 space-y-8">
                    ...
                    <div className="bg-[#0A1128] rounded-[40px] p-8 md:p-12 text-white relative overflow-hidden shadow-2xl">
                        ...
                        <div className="relative z-10">
                            <h1 className="text-2xl md:text-4xl font-black uppercase tracking-tight mb-2">Hoş Geldiniz, <span className="text-indigo-400">Aras İnşaat</span></h1>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            <div className="flex items-center justify-between">
                                ...
                                <button className="text-[10px] font-black text-indigo-600 hover:underline uppercase tracking-widest">TÜMÜNÜ GÖR</button>
                            </div>
                            <div className="space-y-4">
                                {alerts.map(alert => (
                                    <div key={alert.id} className={`p-6 rounded-[32px] border flex items-start gap-6 transition-all hover:translate-x-1 ${alert.type === 'warning' ? 'bg-amber-50 border-amber-100' : alert.type === 'error' ? 'bg-rose-50 border-rose-100' : 'bg-blue-50 border-blue-100'}`}>
                                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${alert.type === 'warning' ? 'bg-amber-200 text-amber-700' : alert.type === 'error' ? 'bg-rose-200 text-rose-700' : 'bg-blue-200 text-blue-700'}`}>
                                            <AlertTriangle size={24} />
                                        </div>
                                        <div className="flex-1">
                                            <h4 className={`text-sm font-black uppercase tracking-tight mb-1 ${alert.type === 'warning' ? 'text-amber-900' : alert.type === 'error' ? 'text-rose-900' : 'text-blue-900'}`}>{alert.title}</h4>
                                            <p className={`text-[11px] font-medium leading-relaxed ${alert.type === 'warning' ? 'text-amber-700/70' : alert.type === 'error' ? 'text-rose-700/70' : 'text-blue-700/70'}`}>{alert.desc}</p>
                                        </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-4 pt-4">
                                    ...
                                    <div className="flex justify-between items-center text-sm font-bold pt-4">
                                        ...
                                        <span className="text-slate-800 text-indigo-600 font-black">6.200.000 ₺</span>
                                    </div>
                                    <div className="w-full h-8 bg-indigo-50 rounded-xl relative overflow-hidden flex items-center px-4">
                                        <div className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">%73 TAMAMLANDI</div>
                                    </div>
                                </div>
                            </div>
