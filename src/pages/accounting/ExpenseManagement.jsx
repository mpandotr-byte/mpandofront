                ...
                <div className="px-4 sm:px-6 md:px-8 pb-12 pt-4 md:pt-6 space-y-8">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                        <div className="relative w-full max-w-lg">
                            ...
                            <input
                                ...
                                className="w-full pl-16 pr-6 py-5 bg-white border border-slate-100 rounded-[32px] text-sm font-bold shadow-sm focus:border-red-400 focus:bg-white outline-none transition-all placeholder:text-slate-300"
                            />
                        </div>
                        <button className="w-full md:w-auto px-10 py-5 bg-red-600 text-white rounded-[24px] text-xs font-black uppercase tracking-[0.2em] shadow-xl shadow-red-600/20 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-3">
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                        {suppliers.map(supplier => (
                            <div
                                ...
                                className={`group relative bg-white rounded-[40px] p-10 border shadow-sm transition-all cursor-pointer hover:shadow-2xl ${selectedSupplier?.id === supplier.id ? 'border-red-500 ring-4 ring-red-50' : 'border-slate-100 hover:border-slate-200'}`}
                            >
                                <div className="flex justify-between items-start mb-10">
                                    <div className="flex items-center gap-6">
                                        <div className="w-16 h-16 rounded-[24px] bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-red-50 group-hover:text-red-500 transition-colors border border-slate-100">
                                            <Truck size={32} />
                                        </div>
                                        <div>
                                ...
                                <div className="flex items-center justify-between pt-10 border-t border-slate-50">
                                    <div className="flex flex-col">
                                        ...
                                        <span className={`text-sm font-black ${supplier.overdue > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                    ...
                    {selectedSupplier && (
                        <div className="bg-white rounded-[40px] border border-slate-200 shadow-2xl overflow-hidden animate-scale-in">
                            <div className="bg-red-600 p-10 text-white flex flex-col md:flex-row justify-between items-center gap-8 shadow-xl shadow-red-600/20">
                                ...
                                <div className="flex gap-4">
                                    ...
                                    <button className="px-8 py-4 bg-white text-red-600 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:scale-105 transition-all">EKSTRE AL</button>
                                </div>
                            </div>

                            {/* Details Tabs */}
                            <div className="flex bg-slate-50 p-2 border-b border-slate-200">
                                <button onClick={() => setActiveTab('agreements')} className={`flex-1 flex items-center justify-center gap-3 py-4 rounded-[22px] text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'agreements' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
                                    <FileText size={18} /> Anlaşmalar
                                </button>
                                <button onClick={() => setActiveTab('invoices')} className={`flex-1 flex items-center justify-center gap-3 py-4 rounded-[22px] text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'invoices' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
                                    <Receipt size={18} /> Fatura & İrsaliye
                                </button>
                                <button onClick={() => setActiveTab('paymentPlan')} className={`flex-1 flex items-center justify-center gap-3 py-4 rounded-[22px] text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'paymentPlan' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
                                    <Calendar size={18} /> Ödeme Takvimi
                                </button>
                                <button onClick={() => setActiveTab('barter')} className={`flex-1 flex items-center justify-center gap-3 py-4 rounded-[22px] text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'barter' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
                            ...
                            <div className="p-10">
                                {activeTab === 'agreements' && (
                                    <div className="space-y-6">
                                        <div className="flex items-center justify-between mb-8">
                                            ...
                                            <button className="flex items-center gap-2 text-[10px] font-black text-red-600 uppercase tracking-widest bg-red-50 px-4 py-2 rounded-xl">+ YENİ PROTOKOL</button>
                                        </div>
                                ...
                                {activeTab === 'invoices' && (
                                    <div className="space-y-8">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <button className="flex items-center gap-2 text-[10px] font-black text-red-600 uppercase tracking-widest bg-red-50 px-4 py-2 rounded-xl">FATURA YÜKLE</button>
                                        </div>
                                        <div className="space-y-4">
                                            {[
                                                ...
                                                <div key={i} className={`p-8 rounded-[32px] border ${inv.mismatch ? 'bg-red-50 border-red-100 shadow-sm' : 'bg-slate-50 border-slate-100'} transition-all`}>
                                                    <div className="flex items-center justify-between mb-6">
                                                        <div className="flex items-center gap-6">
                                                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${inv.mismatch ? 'bg-red-100 text-red-600' : 'bg-white text-slate-400 border border-slate-100'}`}>
                                                                <Receipt size={24} />
                                                            </div>
                                                            <div>
                                                    ...
                                                    {inv.mismatch && (
                                                        <div className="flex items-center gap-3 p-4 bg-white/60 border border-red-200 rounded-2xl">
                                                            <AlertTriangle size={18} className="text-red-500" />
                                                            <div className="text-[11px] font-black text-red-600 uppercase tracking-tighter">FİYAT UYUMSUZLUĞU: Sözleşme (420 ₺) - Fatura (445 ₺)</div>
                                                        </div>
                                                    )}
                                                </div>
                                            ))}
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="space-y-3">
                                            {[
                                                ...
                                                <div key={i} className={`flex items-center justify-between p-6 rounded-3xl border border-slate-100 ${pay.status === 'OVERDUE' ? 'bg-red-50 border-red-100 shadow-sm animate-pulse' : 'bg-slate-50'}`}>
                                                    <div className="flex items-center gap-10">
                                                    ...
                                                    <div className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest ${pay.status === 'PAID' ? 'bg-emerald-50 text-emerald-600' : pay.status === 'OVERDUE' ? 'bg-red-100 text-red-600' : 'bg-orange-50 text-orange-600'}`}>
                                ...
                                {activeTab === 'barter' && (
                                    <div className="flex flex-col items-center justify-center py-20 bg-slate-50 border border-dashed border-slate-200 rounded-[40px]">
                                        <div className="w-20 h-20 rounded-[32px] bg-red-50 text-red-600 flex items-center justify-center mb-6">
                                            <Home size={40} />
                                        </div>
                                        ...
                                        <button className="mt-10 px-10 py-5 bg-red-600 text-white rounded-[24px] text-xs font-black uppercase tracking-[0.2em] shadow-xl hover:scale-105 active:scale-95 transition-all">
                                            DAİRE İLE MAHSUP ET
                                        </button>
                                    </div>
                                )}
